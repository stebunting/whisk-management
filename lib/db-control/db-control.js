...
function dbController(loggingTag, dbName = process.env.DB_NAME) {
  ...
  function getTreatBoxTotals() {
    return new Promise((resolve, reject) => {
      ...
      db.collection('treatboxOrders').aggregate([{
        $group: {
          _id: '$delivery.date', 
          comboBoxes: { $sum: '$itemsOrdered.comboBoxes' },
          treatBoxes: { $sum: '$itemsOrdered.treatBoxes' },
          vegetableBoxes: { $sum: '$itemsOrdered.vegetableBoxes' },
          treatBoxesToMake: {
            $sum: {
              $add: ['$itemsOrdered.treatBoxes', '$itemsOrdered.comboBoxes'] }
            },
          vegetableBoxesToOrder: {
            $sum: {
              $add: ['$itemsOrdered.vegetableBoxes', '$itemsOrdered.comboBoxes'] }
            },
          deliveries: { $sum: { $size: '$recipients' }},
